...
/**
 * movimientos que podremos usar nosotro o algunos el bot
 *
...
public class Movimientos {
    ...
    String[][] tablero; ///// Representa el estado del tablero de ajedrez.

    /**
     *
     * Se utiliza para mantener una copia del tablero original antes de realizar
     * cambios en él.
     *
     * @param arr El tablero al que se copiará el contenido.
     */
    private void copiarTablero(String[][] arr) {
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                arr[i][j] = tablero[i][j]; // Copia cada elemento del tablero actual al tablero dado.
            }
        }
    }

    /**
     * Determina si un movimiento específico de una pieza en el tablero es
     * posible según las reglas del ajedrez.
     *
     * @param tablero El tablero actual del juego.
     * @param posicionAntigua La posición antigua de la pieza que se desea
     * mover.
     * @param posicionNueva La nueva posición a la que se desea mover la pieza.
    ...
    public boolean esPosibleEsteMovimiento(String[][] tablero, String posicionAntigua, String posicionNueva) {
        // Se actualiza el tablero actual con el tablero proporcionado.
        this.tablero = tablero;
        // Se generan los posibles movimientos para la pieza en la posición antigua.
        String[] posicionesPosibles = movimientosAmodificados(tablero, posicionAntigua);
    ...
    /**
     * Actualiza el estado del enroque según la posición antigua de la pieza.
     *
     * @param posAntigua La posición antigua de la pieza que se ha movido.
     */
    private void comprobarAcercaDeEnrroque(String posAntigua) {
        if (posAntigua.equals("74")) {
            autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueReyA = false;
        } else if (posAntigua.equals("70")) {
            autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueTorreIzquierdaA = false;
        } else if (posAntigua.equals("77")) {
            autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueTorreDerechaA = false;
        } else if (posAntigua.equals("04")) {
            autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueReyB = false;
        } else if (posAntigua.equals("00")) {
            autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueTorreIzquierdaB = false;
        } else if (posAntigua.equals("07")) {
            autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueTorreDerechaB = false;
        }
    }

    /**
     * Genera los movimientos posibles para una pieza del jugador A.
     * 
     * @param tableroM El tablero actual del juego.
     * @param posicion La posición de la pieza que se desea mover.
     * @return Un arreglo de cadenas que representa las posiciones a las que la pieza puede moverse.
     */
    ...
    /**
     * Genera los movimientos posibles para una pieza del jugador B.
     * 
     * @param tableroM El tablero actual del juego.
     * @param posicion La posición de la pieza que se desea mover.
     * @return Un arreglo de cadenas que representa las posiciones a las que la pieza puede moverse.
    ...
    /**
     * Modifica los movimientos posibles de acuerdo con ciertas reglas del juego.
     * 
     * @param tableroFuturo El tablero futuro después del movimiento.
     * @param posicionInicial La posición inicial de la pieza que se mueve.
     * @param posicionesFinales Las posiciones a las que la pieza puede moverse inicialmente.
     * @return Un arreglo de cadenas que representa las posiciones finales a las que la pieza puede moverse.
     */
    private String[] modificacionTodosMovimientos(String[][] tableroFuturo, String posicionInicial, String[] posicionesFinales) {
        // Modifica los movimientos posibles de acuerdo con ciertas reglas del juego.
        // Modifica los movimientos posibles de acuerdo con ciertas reglas del je realizan algunas comprobaciones adicionales y modificaciones en los movimientos antes de devolverlos.
    ...
    private String[] movimientoReyA(String[][] tableroM, int y, int x) {
        ...
        //Enrroque 
        if (autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueReyA == true) {
            if (tableroM[7][1].equals("") && tableroM[7][2].equals("") && tableroM[7][3].equals("") && autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueTorreIzquierdaA == true) {
                posicionesPosibles += "" + "72" + "_";
            }
            if (tableroM[7][5].equals("") && tableroM[7][6].equals("") && autonoma.JuegoAjedrez.Controlador.Controlador.enrroqueTorreDerechaA == true) {
