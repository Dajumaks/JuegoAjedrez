package autonoma.JuegoAjedrez.Controlador;

import autonoma.JuegoAjedrez.models.Movimientos;
import autonoma.JuegoAjedrez.models.Bot;
import autonoma.JuegoAjedrez.views.MateHaciaBlancas;
import autonoma.JuegoAjedrez.views.VentanaEleccionFicha;
import autonoma.JuegoAjedrez.views.VistaTablero;
...
/**
 * Controlador del Ajedrez nos ayuda a maniobrar con diferentes cosas dentro de
 * esta clase
 *
 ...
 */
////////////////////////////////////////////
///implements se utiliza para declarar que una clase implementa una o varias interfaces
public class Controlador implements ActionListener {
    ////////////////////
    /// Se están declarando varias variables booleanas estáticas para 
    ///controlar si se pueden realizar enroques en el juego

    ////////////El enroque es un movimiento especial en el juego de ajedrez que implica mover simultáneamente el rey y una de las torres en una jugada
    ...
    //////////////////
    /////////tablero es un arreglo bidimensional de cadenas (String) que se utiliza para representar el estado actual del tablero de ajedrez
    public static String[][] tablero = new String[8][8];
    //////Se declaran varias variables para controlar el estado del juego
    private char turnoJugador = 'A';
    private String posicionAntigua = null;
    private String posicionNueva = null;
    private String posicionActual;
    /////////////// 
    /////Se declaran variables estáticas para almacenar la ficha seleccionada por el jugador y su imagen correspondiente
    public static String fichaElegida;
    public static ImageIcon imagenElegida;
    Movimientos movimientos;
    ///////////////
    ////Se inicializa un objeto Bot, representa la inteligencia artificial del juego para el jugador de la computadora.
    Bot bot = new Bot();

    ///////////////////////
    /////este constructor se encarga de inicializar varios aspectos del juego de ajedrez, como el tablero, la vista en la interfaz de usuario y la lógica de los movimientos
    public Controlador() {
        iniciarTablero();
    ...
    ////////////////
    // se encarga de inicializar el estado del tablero de ajedrez en el juego.
    private void iniciarTablero() {
        /////////////// itera sobre las filas del tablero (índice i) y el segundo bucle for itera sobre las columnas del tablero (índice j).
        for (int i = 0; i < 8; i++) {
            for (int j = 0; j < 8; j++) {
                tablero[i][j] = "";
            }
        ...
        /////////se utiliza para colocar los peones en las filas 1 y 6 del tablero, que son las filas donde se colocan los peones al comienzo del juego
        for (int i = 0; i < 8; i++) {
            tablero[1][i] = "B_peon";
            tablero[6][i] = "A_peon";
        }

        /////////// inicializa el tablero de ajedrez con las piezas colocadas en sus posiciones iniciales, listo para comenzar una partida.
        tablero[0][0] = "B_torre";
    ...
    /////////La razón por la que se asigna un ActionListener a cada botón es para que puedan responder a eventos de acción,
    ////////////como hacer clic sobre ellos. Cuando un botón se activa (por ejemplo, haciendo clic sobre él), se generará 
    ////////////un evento de acción que será manejado por el ActionListener asociado.
    ...
    ////////////para cada botón individualmente, lo cual es tedioso y propenso a errores si hay cambios en el número o la nomenclatura de los botones.
    /////////Una forma más fácil de hacerlo es utilizar bucles anidados para iterar 
    ////////sobre las filas y columnas del tablero y agregar dinámicamente los ActionListener a cada botón.
//
//    private void añadirActionEvents() {
//        for (int i = 0; i < 8; i++) {
//            for (int j = 0; j < 8; j++) {
//                VistaTablero.botones[i][j].addActionListener(this);
//            }
//        }
//    }
    @Override
    //////este método maneja los eventos de acción relacionados con el movimiento de las fichas en el juego de ajedrez, 
    ///asegurándose de que se realicen movimientos válidos y gestionando el flujo del juego entre los jugadores.
    ...
    ////////////
    ///Al final de la ejecución de este método, la variable jaqueMate indicará si el jugador blanco está en jaque mate (true) o no (false). Si jaqueMate permanece como true, 
    /////significa que el jugador blanco está en jaque mate. De lo contrario, 
    /////significa que hay al menos un movimiento posible para evitar el jaque mate.
    ...
    //////este bloque de código muestra una ventana emergente si se detecta que el jugador blanco está en jaque mate
        if (jaqueMate == true) {
    ...
    //////permite a la máquina realizar un movimiento en el juego de ajedrez
    ...
    /////////coordina varias acciones relacionadas con el movimiento de una ficha en el juego de ajedrez, 
    /////incluida la actualización de la representación interna y visual del tablero, 
    /////la verificación de si se debe promocionar un peón y la comprobación de si se ha realizado un enroque.
    ...
    ////////este método actualiza la representación interna del tablero de ajedrez para reflejar el movimiento de una ficha desde una posición antigua a una nueva posición.
    ...
    ///////este método actualiza la representación visual del tablero de ajedrez en la interfaz 
    /////de usuario para reflejar el movimiento de una ficha desde una posición antigua a una nueva posición
    ...
    ///////este método maneja la promoción automática de peones que alcanzan la última fila del tablero
    //////y actualiza la representación del tablero en la interfaz de usuario con la pieza promocionada correspondiente
    ...
    ///////crea y muestra una ventana de elección de ficha, permitiendo al jugador elegir a qué pieza desea promocionar su peón cuando llegue a la última fila del tablero
    ...
    //////////verifica si se ha realizado un enroque, y si es así, actualiza el tablero y la interfaz de usuario para reflejar el movimiento de las piezas involucradas en el enroque.
    //////Esto se hace moviendo la torre adecuada a la nueva posición y actualizando los iconos de los botones correspondientes en la interfaz de usuario
    ...
    ///////////7Este tipo de estructura se utiliza cuando cada posición del tablero tiene un identificador único y predefinido 
    /////////////que se corresponde con un componente específico en la interfaz de usuario
...
////////////////Este tipo de estructura se utiliza cuando cada posición del tablero tiene un identificador único y 
    ////predefinido que se corresponde con un componente específico en la interfaz de usuario
    ...
    //////////este método determina si la ficha en una posición dada es blanca basándose en el contenido de la matriz tablero. 
    ///////Si la casilla está vacía o si la ficha no es blanca, devuelve false; de lo contrario, devuelve true.
